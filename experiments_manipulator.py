from Manipulator import TaskSpaceManipulator
...
def run_experiment_p_control(kp, eps, target_com=[0, 0.01, -0.036]):
    kd = 0
    task_space = TaskSpaceManipulator("kuka_iiwa/model.urdf", kp, kd)
...
if __name__ == "__main__":
    tgt = [0.4, 0.1, 0.3]
    kp_arr = [5,10,15,50,100,200]
    second_par  = [0.01,0.1,1,10,100]
    
    print("=================Start main loop=================")
    
    for i in range(len(kp_arr)):
        run_experiment_p_control(kp_arr[i], 10**-4,tgt)
        
    for i in range(len(second_par)):
        run_experiment_damped(10, 10**-4,second_par[i],tgt)
        run_experiment_null_space(10, 10**-4,second_par[i],tgt)
